...
transformed parameters {
  ...
  for (n in 1:N) {
    theta[n,1] = exp(adj_h_eff[instants[n]] + att[instants[n], team1[n]] + def[instants[n], team2[n]] +
...
model{
  ...
  // Priors on NB dispersion
  target += normal_lpdf(phi1 | 0, 5);
  target += normal_lpdf(phi2 | 0, 5);

  // Likelihood: marginal NB2 for each margin
  for (n in 1:N) {
...
generated quantities {
  ...
  for (n in 1:N) {
    y_rep[n,1] = neg_binomial_2_rng(fmin(theta[n,1], max_rate), phi1);
    y_rep[n,2] = neg_binomial_2_rng(fmin(theta[n,2], max_rate), phi2);
  ...
  if (N_prev > 0) {
    for (n in 1:N_prev) {
      theta_prev[n,1] = exp(adj_h_eff[instants_prev[n]] + att[instants_prev[n], team1_prev[n]] +
      ...
      theta_prev[n,2] = exp(att[instants_prev[n], team2_prev[n]] +
                           ...
                           (gamma/2)*(ranking[instants_rank[N], team1_prev[n]] - ranking[instants_rank[N], team2_prev[n]]));
      y_prev[n,1] = neg_binomial_2_rng(fmin(theta_prev[n,1], max_rate), phi1);
      y_prev[n,2] = neg_binomial_2_rng(fmin(theta_prev[n,2], max_rate), phi2);
    }
  }
}
