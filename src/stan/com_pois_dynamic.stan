functions {
  ...
  real com_poisson_lpmf(int y, real lambda, real nu) {
    real log_normalizer;
    int max_sum = 100;  // Maximum terms for approximation
    vector[max_sum + 1] log_terms;

    // Compute normalizing constant approximation
    for (i in 0:max_sum) {
      log_terms[i + 1] = i * log(lambda) - nu * lgamma(i + 1);
    }
    log_normalizer = log_sum_exp(log_terms);

    // Return log probability
    return y * log(lambda) - nu * lgamma(y + 1) - log_normalizer;
  }

  // COM-Poisson random number generator
  int com_poisson_rng(real lambda, real nu) {
    int max_y = 100;
    vector[max_y + 1] log_probs;

    // Compute unnormalized log probabilities
    for (y in 0:max_y) {
      log_probs[y + 1] = y * log(lambda) - nu * lgamma(y + 1);
    }

    // Normalize and sample
    return categorical_rng(softmax(log_probs)) - 1;
  }
}
...
data {
  ...
  // // COM-Poisson specific
  // real<lower=0> hyper_nu_mean;      // Prior mean for dispersion
  // real<lower=0> hyper_nu_sd;        // Prior SD for dispersion
}
...
parameters {
  ...
  vector[ntimes] home;
  real gamma;
  array[(ind_comm_prior|| ind_common_sigma) ? 0 : 1] real<lower=0> sigma_att;
  array[(ind_comm_prior|| ind_common_sigma) ? 0 : 1] real<lower=0> sigma_def;
  array[(ind_comm_prior|| !ind_common_sigma) ? 0 : 1] real<lower=0> sigma_common;
  ...
  real<lower=0> nu;  // nu=1: Poisson, nu<1: overdispersion, nu>1: underdispersion
}

transformed parameters {
  vector[ntimes] adj_h_eff;
  matrix[ntimes, nteams] att;
  matrix[ntimes, nteams] def;
  matrix[ntimes, nteams] mu_att;
  matrix[ntimes, nteams] mu_def;
  vector[N] theta_home;
  vector[N] theta_away;
  ...
  for (n in 1:N) {
    theta_home[n] = exp(adj_h_eff[instants[n]] + att[instants[n], team1[n]] +
                        def[instants[n], team2[n]] +
  ...
  array[ind_comm_prior ? ntimes : 0, ind_comm_prior ? nteams : 0] real<lower=0> comm_sd_att = inv_sqrt(comm_prec_att);
  array[ind_comm_prior ? ntimes : 0, ind_comm_prior ? nteams : 0] real<lower=0> comm_sd_def = inv_sqrt(comm_prec_def);
}
...
model {
  if (ind_comm_prior == 1) {
    ...
    for (h in 1:nteams) {
      for (i in 1:ntimes) {
        if (prior_dist_num == 1) {
          target += normal_lpdf(att_raw[i,h] | mu_att[i,h], comm_sd_att[i,h]);
          target += normal_lpdf(def_raw[i,h] | mu_def[i,h], comm_sd_def[i,h]);
        }
        else if (prior_dist_num == 2) {
          target += student_t_lpdf(att_raw[i,h] | hyper_df, mu_att[i,h], comm_sd_att[i,h]);
          target += student_t_lpdf(def_raw[i,h] | hyper_df, mu_def[i,h], comm_sd_def[i,h]);
        }
        else if (prior_dist_num == 3) {
          target += student_t_lpdf(att_raw[i,h] | 1, mu_att[i,h], comm_sd_att[i,h]);
          target += student_t_lpdf(def_raw[i,h] | 1, mu_def[i,h], comm_sd_def[i,h]);
        }
      }
    }
  }
  ...
  // Prior for COM-Poisson dispersion parameter
  target += normal_lpdf(nu | 1, 1);

  // COM-Poisson likelihood
  for (n in 1:N) {
    target += com_poisson_lpmf(y[n,1] | theta_home[n], nu);
    target += com_poisson_lpmf(y[n,2] | theta_away[n], nu);
  }
}
...
generated quantities {
  ...
  for (n in 1:N) {
    y_rep[n,1] = com_poisson_rng(theta_home[n], nu);
    y_rep[n,2] = com_poisson_rng(theta_away[n], nu);
    diff_y_rep[n] = y_rep[n,1] - y_rep[n,2];
    log_lik[n] = com_poisson_lpmf(y[n,1] | theta_home[n], nu) +
                 com_poisson_lpmf(y[n,2] | theta_away[n], nu);
  }

  // Out-of-sample predictions
  if (N_prev > 0) {
    for (n in 1:N_prev) {
      theta_home_prev[n] = exp(adj_h_eff[instants_prev[n]] +
                               att[instants_prev[n], team1_prev[n]] +
      ...
      y_prev[n,1] = com_poisson_rng(theta_home_prev[n], nu);
      y_prev[n,2] = com_poisson_rng(theta_away_prev[n], nu);
    }
  }
}
