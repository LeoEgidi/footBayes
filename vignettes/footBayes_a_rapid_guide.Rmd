...
vignette: >
  %\VignetteIndexEntry{Fitting football models with the ```footBayes``` package}
...
To say in art's terms, I love this quote from Antoine de Saint-Exupéry (partially rephrased):
...
```{r static_fit, echo =TRUE, eval = TRUE}
...
n_iter <- 1500    # number of MCMC iterations
...
We can also access the original Stan code by typing:
...
```{r static_fit_priors, echo =TRUE, eval = TRUE}
### Fit Stan models
## changing priors
...
```{r dynamic_fit, echo =TRUE, eval = TRUE}
### Fit Stan models
...
```{r weekly_fit, echo = TRUE, eval = TRUE}
### Fit Stan models
...
```{r weekly_fit_t, echo = TRUE, eval = TRUE}
### Fit Stan models
## weekly dynamics, no predictions
## 4 chains, 'n_iter' iterations each
...
```{r abilities, echo = TRUE, eval = TRUE, fig.show="hold", out.width="90%"}
...
```{r abilities_dyn, echo = TRUE, eval = TRUE, fig.show="hold", out.width="90%"}
## Plotting abilities: credible and confidence 95% intervals
...
The aggregated goal difference frequencies seem to be decently captured by the model's replications: in the first plot, blue horizontal lines denote the observed goal differences frequencies registered in the dataset, wehreas yellow jittered points denote the correspondent replications. Goal-difference of 0, corresponding to the draws occurrences, is only slightly underestimated by the model. However, in general there are no particular clues of model's misfit.

In the second plot, the ordered observed goal differences are plotted against their replications (50% and 95% credible intervals), and from this plot also we have not particular signs of model's misfits.
...
```{r weekly_predict, echo=TRUE, eval= TRUE}
### Fit Stan models
...
where $(\tilde{x}^{(s)}, \tilde{y}^{(s)})$ represents the $s$-th MCMC pair of the future home goals and away goals for a given match, respectively. According to this scenario, we can depict the home-win posterior probabilities of a given test set through the function ```foot_round_robin``` (yet under development!)
...
```{r rank_pred1, echo=TRUE, eval= TRUE,  out.width="90%"}
## Rank league reconstruction
...
foot_rank(data = italy_2000, object = fit1_stan, visualize = "single teams")
...
```{r rank_pred2, echo=TRUE, eval= TRUE, out.width="90%"}
...
Extensions and to-do list for the final package's version:
