...
The Bradley-Terry model <span style="color:red">[@bradley_Terry1952] </span> is one of the most popular modelling techniques in a pairwise comparison context for ranking players or teams. The model assumes that each team $T_k$, with $k = 1, \dots, N_T $,
...
the ratio of these parameters. Specifically, for a match between team $T_i$ and team $T_j$, with $i\neq j = 1, \dots, N_T $ , the probability that $T_i$ defeats $T_j$ in the \textit{n}-th match, with $n = 1, \dots, N$, is 
\begin{align}
...
The standard Bradley-Terry model does not account for draws. To address this limitation, several alternatives have been proposed. <span style="color:red">@davidson1970 </span> introduced an additional parameter, \zeta, that balances the probability of ties against the probability of not having ties, thereby computing two distinct probabilities. The log-parametrization of the BTD model is 
\begin{align}
...
regression coefficients. Furthermore, $\omega_n = (rank\_points_{h_n}-rank\_points_{a_n})$ captures the **difference in relative strengths** (ranking points) between the home and away teams in the  \textit{n}-th match. Finally, the parameter $\gamma$ tries to correct for the ranking points difference occurring between two competing teams. The parameters $\phi_1, \phi_2$ and $\phi_3$ are dummy binary indicators taking values 0 or 1 which may activate distinct sources of the linear 
...
```{r BTD_data, echo = TRUE, eval = NOT_CRAN}
library(dplyr)
library(footBayes) 
...
- ```dynamic_rank = FALSE``` for static Bayesian BTD model.
- ```dynamic_rank = TRUE``` for dynamic Bayesian BTD model.
...
```{r BTD_model_1, message = FALSE, results='hide', echo = TRUE, eval = NOT_CRAN}
...
print(fit_result_dyn, 
      ...
      teams = c("AC Milan", "AS Roma", "FC Crotone"))
...
```{r BTD_model_2, message = FALSE, results='hide', echo = TRUE, eval = NOT_CRAN}
# Dynamic Ranking Example with Median Rank Measure
...
```{r plot_btdPosterior_dyn, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
```{r plot_btdPosterior_stat, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
```{r plot_btdPosterior_teams_dyn, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
```{r plot_btdPosterior_teams_stat, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
```{r plot_btdPosterior_teams_dyn_dens, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
```{r plot_btdPosterior_teams_stat_dens, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
```{r plot_logStrength_teams_dyn, echo = TRUE, eval = NOT_CRAN, fig.show="hold"}
...
foot_rank(fit4_stan, italy_2000,
          teams = c("AC Milan", "AS Roma"), 
          visualize = "individual")
