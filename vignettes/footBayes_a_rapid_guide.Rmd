...
The ```btd_foot``` function fits a Bayesian Bradley-Terry-Davidson (BTD) model by generating posterior samples using Hamiltonian Monte Carlo within the underlying ```rstan``` ecosystem. It supports both static and dynamic ranking models, allowing for the estimation of team strengths over time.
...
\begin{align}
\psi_{t,\tau} \sim \mathrm{N}(\mu_{\psi_{t-1,\tau}}, \sigma_\psi)
\end{align}
...
#> 1 logStrength[1, AC Milan]  2.17   2.17  0.934 0.949  0.635 3.67   1.00    165.      351.
#> 2 logStrength[2, AC Milan]  1.37   1.38  1.07  1.10  -0.411 3.15   1.01    103.      307.
#> 3 logStrength[3, AC Milan]  2.37   2.4   1.25  1.23   0.22  4.38   1.00     72.0     201.
#> 4 logStrength[4, AC Milan]  2.85   2.93  1.48  1.50   0.278 5.22   1.00     68.8     183.
#> 5 logStrength[1, AS Roma]   1.48   1.47  0.921 0.937 -0.027 3.00   1.00    158.      633.
#> 6 logStrength[2, AS Roma]  -0.04  -0.049 1.06  1.09  -1.83  1.78   1.01     91.7     289.
#> 7 logStrength[3, AS Roma]   0.886  0.937 1.21  1.19  -1.24  2.80   1.00     73.5     174.
#> 8 logStrength[4, AS Roma]   1.04   1.12  1.40  1.40  -1.40  3.31   1.01     60.2     174.
#> 9 logTie                   -0.012 -0.014 0.085 0.083 -0.152 0.128  1.00   1343.     1570.
```
...
#> 7        1        AC Milan         2.086
#> 6        1        Juventus         1.781
#> 13       1      SSC Napoli         1.711
#> 20       1        Atalanta         1.287
#> 19       1      Lazio Roma         1.104
#> 15       1         AS Roma         0.932
#> 5        1 Sassuolo Calcio         0.815
#> 1        1  ACF Fiorentina         0.239
#> 2        1   Hellas Verona         0.160
#> 
#> Posterior summaries for model parameters:
#> # A tibble: 3 × 10
#>   variable                mean median    sd   mad     q5   q95  rhat ess_bulk ess_tail
#>   <chr>                  <dbl>  <dbl> <dbl> <dbl>  <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 logStrength[AC Milan]  2.00   2.02  0.71  0.734  0.778 3.13   1.01     117.     236.
#> 2 logStrength[AS Roma]   0.818  0.846 0.682 0.724 -0.345 1.90   1.01     104.     219.
#> 3 logTie                -0.114 -0.114 0.09  0.091 -0.265 0.031  1.00     951.     997.
```
...
#>   variable  mean median    sd   mad     q5   q95  rhat ess_bulk ess_tail
#>   <chr>    <dbl>  <dbl> <dbl> <dbl>  <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 logTie   0.022  0.02  0.089 0.088 -0.127 0.168  1       2281.    1285.
#> 2 home     0.232  0.234 0.111 0.112  0.048 0.414  1.00    2798.    1220.
```
...
#> 11       1           Inter          4.66
#> 7        1        AC Milan          4.12
#> 13       1      SSC Napoli          3.83
#> 6        1        Juventus          3.64
#> 20       1        Atalanta          3.35
#> 19       1      Lazio Roma          3.05
#> 15       1         AS Roma          3.03
#> 5        1 Sassuolo Calcio          2.67
#> 1        1  ACF Fiorentina          2.53
#> 2        1   Hellas Verona          2.29
#> 
#> Posterior summaries for model parameters:
#> # A tibble: 2 × 10
#>   variable   mean median    sd   mad     q5   q95  rhat ess_bulk ess_tail
#>   <chr>     <dbl>  <dbl> <dbl> <dbl>  <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 logTie   -0.11  -0.112 0.089 0.084 -0.256 0.039  1.00     432.     447.
#> 2 home      0.214  0.213 0.094 0.095  0.058 0.366  1.00     447.     518.
```
...
#>   variable        mean median    sd   mad     q5    q95  rhat ess_bulk ess_tail
#>   <chr>          <dbl>  <dbl> <dbl> <dbl>  <dbl>  <dbl> <dbl>    <dbl>    <dbl>
#> 1 home           0.278  0.285 0.069 0.072  0.161  0.382  1.00     675.     561.
#> 2 rho           -1.74  -1.70  0.32  0.313 -2.34  -1.31   1.01     847.     690.
#> 3 sigma_att      0.219  0.211 0.069 0.065  0.12   0.342  1.00     415.     301.
#> 4 sigma_def      0.227  0.219 0.077 0.072  0.121  0.359  1.00     339.     345.
#> 5 att[AS Roma]   0.281  0.281 0.127 0.125  0.076  0.489  1.01     752.     700.
#> 6 att[AC Milan]  0.134  0.133 0.123 0.124 -0.071  0.34   1.01    1409.     678.
#> 7 def[AS Roma]  -0.203 -0.193 0.149 0.144 -0.474  0.029  1.00    1346.     523.
#> 8 def[AC Milan] -0.001  0.005 0.135 0.119 -0.239  0.211  1.01    1691.     591.
```
...
#>  [30] "      array[N,2] int y;"                                                                                       
#>  [31] "      int nteams;"                                                                                             
#>  [32] "      array[N]int instants_rank;"                                                                              
#>  [33] "      int ntimes_rank;                 // dynamic periods for ranking"                                         
#>  [34] "      array[N] int team1;"                                                                                     
#>  [35] "      array[N] int team2;"                                                                                     
#>  [36] "      matrix[ntimes_rank,nteams] ranking;"                                                                     
#>  [37] "      int<lower=0, upper=1> ind_home;"                                                                         
#>  [38] "      real mean_home;              // Mean for home effect"                                                    
#>  [39] "      real<lower=0> sd_home;      // Standard deviation for home effect"                                       
#>  [40] ""                                                                                                              
#>  [41] "      // priors part"                                                                                          
#>  [42] "      int<lower=1,upper=4> prior_dist_num;    // 1 gaussian, 2 t, 3 cauchy, 4 laplace"                         
#>  [43] "      int<lower=1,upper=4> prior_dist_sd_num; // 1 gaussian, 2 t, 3 cauchy, 4 laplace"                         
#>  [44] ""                                                                                                              
#>  [45] "      real hyper_df;"                                                                                          
#>  [46] "      real hyper_location;"                                                                                    
#>  [47] ""                                                                                                              
#>  [48] "      real hyper_sd_df;"                                                                                       
#>  [49] "      real hyper_sd_location;"                                                                                 
#>  [50] "      real hyper_sd_scale;"                                                                                    
#>  [51] "    }"                                                                                                         
#>  [52] "    parameters{"                                                                                               
#>  [53] "      vector[nteams] att_raw;"                                                                                 
#>  [54] "      vector[nteams] def_raw;"                                                                                 
#>  [55] "      real<lower=0> sigma_att;"                                                                                
#>  [56] "      real<lower=0> sigma_def;"                                                                                
#>  [57] "      //real beta;"                                                                                            
#>  [58] "      real rho;"                                                                                               
#>  [59] "      real home;"                                                                                              
#>  [60] "      real gamma;"                                                                                             
...
#>  [63] "      real adj_h_eff;                   // Adjusted home effect"                                               
#>  [64] "      vector[nteams] att;"                                                                                     
#>  [65] "      vector[nteams] def;"                                                                                     
#>  [66] "      array[N] vector[3] theta;"                                                                               
#>  [67] ""                                                                                                              
#>  [68] "      for (t in 1:nteams){"                                                                                    
#>  [69] "        att[t] = att_raw[t]-mean(att_raw);"                                                                    
#>  [70] "        def[t] = def_raw[t]-mean(def_raw);"                                                                    
#>  [71] "      }"                                                                                                       
#>  [72] ""                                                                                                              
#>  [73] "      adj_h_eff = home * ind_home;"                                                                            
#>  [74] ""                                                                                                              
#>  [75] "      for (n in 1:N){"                                                                                         
#>  [76] "        theta[n,1] = exp(adj_h_eff+att[team1[n]]+def[team2[n]]+"                                               
#>  [77] "                         (gamma/2)*(ranking[instants_rank[n], team1[n]]-ranking[instants_rank[n], team2[n]]));"
#>  [78] "        theta[n,2] = exp(att[team2[n]]+def[team1[n]]-"                                                         
#>  [79] "                         (gamma/2)*(ranking[instants_rank[n], team1[n]]-ranking[instants_rank[n], team2[n]]));"
#>  [80] "        theta[n,3] = exp(rho);"                                                                                
#>  [81] "      }"                                                                                                       
#>  [82] "    }"                                                                                                         
#>  [83] "    model{"                                                                                                    
#>  [84] "      // log-priors for team-specific abilities"                                                               
#>  [85] "      for (t in 1:(nteams)){"                                                                                  
#>  [86] "        if (prior_dist_num == 1){"                                                                             
#>  [87] "          target+= normal_lpdf(att_raw[t]|hyper_location, sigma_att);"                                         
#>  [88] "          target+= normal_lpdf(def_raw[t]|hyper_location, sigma_def);"                                         
#>  [89] "        }"                                                                                                     
#>  [90] "        else if (prior_dist_num == 2){"                                                                        
#>  [91] "          target+= student_t_lpdf(att_raw[t]|hyper_df, hyper_location, sigma_att);"                            
#>  [92] "          target+= student_t_lpdf(def_raw[t]|hyper_df, hyper_location, sigma_def);"                            
#>  [93] "        }"                                                                                                     
#>  [94] "        else if (prior_dist_num == 3){"                                                                        
#>  [95] "          target+= cauchy_lpdf(att_raw[t]|hyper_location, sigma_att);"                                         
#>  [96] "          target+= cauchy_lpdf(def_raw[t]|hyper_location, sigma_def);"                                         
#>  [97] "        }"                                                                                                     
#>  [98] "        else if (prior_dist_num == 4){"                                                                        
#>  [99] "          target+= double_exponential_lpdf(att_raw[t]|hyper_location, sigma_att);"                             
#> [100] "          target+= double_exponential_lpdf(def_raw[t]|hyper_location, sigma_def);"                             
#> [101] "        }"                                                                                                     
#> [102] "      }"                                                                                                       
#> [103] ""                                                                                                              
#> [104] ""                                                                                                              
#> [105] "      // log-hyperpriors for sd parameters"                                                                    
#> [106] "      if (prior_dist_sd_num == 1 ){"                                                                           
#> [107] "        target+=normal_lpdf(sigma_att|hyper_sd_location, hyper_sd_scale);"                                     
#> [108] "        target+=normal_lpdf(sigma_def|hyper_sd_location, hyper_sd_scale);"                                     
#> [109] "      }"                                                                                                       
#> [110] "      else if (prior_dist_sd_num == 2){"                                                                       
#> [111] "        target+=student_t_lpdf(sigma_att|hyper_sd_df, hyper_sd_location, hyper_sd_scale);"                     
#> [112] "        target+=student_t_lpdf(sigma_def|hyper_sd_df, hyper_sd_location, hyper_sd_scale);"                     
#> [113] "      }"                                                                                                       
#> [114] "      else if (prior_dist_sd_num == 3){"                                                                       
#> [115] "        target+=cauchy_lpdf(sigma_att|hyper_sd_location, hyper_sd_scale);"                                     
#> [116] "        target+=cauchy_lpdf(sigma_def|hyper_sd_location, hyper_sd_scale);"                                     
#> [117] "      }"                                                                                                       
#> [118] "      else if (prior_dist_sd_num == 4){"                                                                       
#> [119] "        target+=double_exponential_lpdf(sigma_att|hyper_sd_location, hyper_sd_scale);"                         
#> [120] "        target+=double_exponential_lpdf(sigma_def|hyper_sd_location, hyper_sd_scale);"                         
#> [121] "      }"                                                                                                       
#> [122] ""                                                                                                              
#> [123] "      // log-priors fixed effects"                                                                             
#> [124] "      target+=normal_lpdf(rho|0,1);"                                                                           
#> [125] "      target+=normal_lpdf(home|mean_home,sd_home);"                                                            
#> [126] "      target+=normal_lpdf(gamma|0,1);"                                                                         
#> [127] ""                                                                                                              
#> [128] "      // likelihood"                                                                                           
#> [129] "      for (n in 1:N){"                                                                                         
#> [130] "         target+=poisson_lpmf(y[n,1]|theta[n,1]+theta[n,3]);"                                                  
#> [131] "         target+=poisson_lpmf(y[n,2]|theta[n,2]+theta[n,3]);"                                                  
#> [132] "       //  target+=bipois_lpmf(y[n,]| theta[n,1],"                                                             
#> [133] "          //                   theta[n,2], theta[n,3]);"                                                       
#> [134] "      }"                                                                                                       
#> [135] "    }"                                                                                                         
#> [136] "    generated quantities{"                                                                                     
#> [137] "      array[N,2] int y_rep;"                                                                                   
#> [138] "      vector[N] log_lik;"                                                                                      
#> [139] "      array[N] int diff_y_rep;"                                                                                
#> [140] ""                                                                                                              
#> [141] "      //in-sample replications"                                                                                
#> [142] "      for (n in 1:N){"                                                                                         
#> [143] "        y_rep[n,1] = poisson_rng(theta[n,1]+theta[n,3]);"                                                      
#> [144] "        y_rep[n,2] = poisson_rng(theta[n,2]+theta[n,3]);"                                                      
#> [145] "        diff_y_rep[n] = y_rep[n,1] - y_rep[n,2];"                                                              
#> [146] "        log_lik[n] = poisson_lpmf(y[n,1]|theta[n,1]+theta[n,3])+"                                              
#> [147] "                     poisson_lpmf(y[n,2]|theta[n,2]+theta[n,3]);"                                              
#> [148] "       //log_lik[n] =bipois_lpmf(y[n,]| theta[n,1],"                                                           
#> [149] "                //                 theta[n,2], theta[n,3]);"                                                   
#> [150] "      }"                                                                                                       
#> [151] "    }"
```
...
#>   variable    mean median    sd   mad     q5    q95  rhat ess_bulk ess_tail
#>   <chr>      <dbl>  <dbl> <dbl> <dbl>  <dbl>  <dbl> <dbl>    <dbl>    <dbl>
#> 1 home       0.277  0.279 0.068 0.067  0.172  0.385  1.01   583.      237. 
#> 2 rho       -1.78  -1.74  0.34  0.283 -2.41  -1.27   1.00   452.      327. 
#> 3 sigma_att  0.046  0.044 0.012 0.011  0.03   0.069  1.54     3.78     21.5
#> 4 sigma_def  0.058  0.056 0.014 0.013  0.036  0.084  1.62     3.49     12.1
```
...
#>   variable   mean median    sd   mad    q5   q95  rhat ess_bulk ess_tail
#>   <chr>     <dbl>  <dbl> <dbl> <dbl> <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 home      0.413  0.414 0.04  0.041 0.352 0.478  1.02   418.      359. 
#> 2 sigma_att 0.04   0.039 0.007 0.006 0.029 0.054  1.69     3.50     22.1
#> 3 sigma_def 0.055  0.052 0.013 0.012 0.037 0.08   1.61     3.54     21.6
```
...
#>   variable   mean median    sd   mad    q5   q95  rhat ess_bulk ess_tail
#>   <chr>     <dbl>  <dbl> <dbl> <dbl> <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 home      0.398  0.402 0.05  0.051 0.32  0.474  1.03    74.4     202. 
#> 2 sigma_att 0.034  0.031 0.01  0.011 0.021 0.053  1.82     3.21     39.2
#> 3 sigma_def 0.055  0.053 0.011 0.01  0.04  0.077  1.21     7.25     62.5
```
...
#>   variable    mean median    sd   mad     q5    q95  rhat ess_bulk ess_tail
#>   <chr>      <dbl>  <dbl> <dbl> <dbl>  <dbl>  <dbl> <dbl>    <dbl>    <dbl>
#> 1 home       0.163  0.164 0.07  0.066  0.044  0.282  1.01   692.      378. 
#> 2 rho       -1.10  -1.1   0.16  0.151 -1.38  -0.856  1.00  1202.      728. 
#> 3 sigma_att  0.035  0.03  0.023 0.023  0.006  0.079  1.20     7.29     33.7
#> 4 sigma_def  0.029  0.022 0.021 0.018  0.006  0.07   1.18     8.23     53.5
```
...
#> 1 Reggina Calcio  AC Milan  0.331  0.258  0.411 1-1 (0.131)
#> 
#> $prob_plot
```
...
#>  2 Inter          Bologna FC     0.428     -       
#>  3 Udinese Calcio SSC Napoli     0.5       -       
#>  4 ACF Fiorentina SSC Napoli     0.552     -       
#>  5 US Lecce       Lazio Roma     0.282     -       
#>  6 Inter          Lazio Roma     0.292     -       
#>  7 Brescia Calcio AS Bari        0.655     -       
#>  8 Reggina Calcio AS Bari        0.627     -       
#>  9 Udinese Calcio Vicenza Calcio 0.405     -       
#> 10 Brescia Calcio Vicenza Calcio 0.478     -       
#> # ℹ 26 more rows
#> 
#> $round_plot
```
...
#>     biv_pois 0.255    0.342 0.719    0.309 0.347
#>  double_pois 0.255    0.358 0.714    0.308 0.361
#> 
#> Confusion Matrices
#> Model: biv_pois 
#> 
